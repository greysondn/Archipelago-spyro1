...
class BHMemoryEntry:
    def __init__(self, address, length):
        self.address = f"0x{address:05x}" if isinstance(address, int) else address
        self.length = length

class Portal:
    def __init__(self):
        self.destination_address = "0x0"
...
class SpyroHub:
    def __init__(self):
        self.name = "ERR"
        self.id = -1
        self.balloon_addresses:tuple[str, str] = ("-1", "-1")
        self.text_offset = "0x0"
        self.total_gems = -1
        self.gem_counter = BHMemoryEntry("0x0", 0)
        self.dragons = []
        self.eggs = []
        self.levels = []
    ...
    def add_dragon(self, name, address, value):
        address = f"0x{address:05x}" if isinstance(address, int) else address
...
artisans.statue_head_checks = ["0x7f48c", "0x7f4c8"]
world.add_hub(artisans)

peace_keepers = SpyroHub()
...
print(jsonpickle.encode(world, unpicklable=False, indent=4))